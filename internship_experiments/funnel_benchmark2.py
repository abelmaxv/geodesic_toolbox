...
from geodesic_toolbox import ImplicitFHMCUnbiased, ImplicitRHMCSampler, FunnelDualRanders, FunnelSoftAbs
...
class FunnelHMC:
    def __init__(self, dim: int, step_size: float, mass_variance: float, N_run: int, l: int = 1, burn: int = 200):
        self.dim           = dim
    ...
    def log_prob(self, z: Tensor) -> Tensor:
        v     = z[0]
        theta = z[1:]
        return -(v**2 / 18 + (self.dim / 2) * v + torch.exp(-v) / 2 * (theta**2).sum())
...
def target_function(
    ...
    z_0  = torch.zeros(N_batch, state_dim)
...
def make_fhmc_unbiased_factory(dim: int, reduced_flip: bool = True):
    def factory(parameters: Tensor, N_run: int):
        p = parameters.squeeze()
        return FunnelFHMCUnbiased(
            dim=dim, l=1, N_fx=6,
...
HMC_CONFIG = {
    "sampler_factory" : make_hmc_factory(DIM),
...
RHMC_CONFIG = {
    "sampler_factory" : make_rhmc_factory(DIM),
    "objective_fn"    : min_ess_objective,
...
FHMC_REDUCED = {
    "sampler_factory" : make_fhmc_unbiased_factory(DIM, reduced_flip=True),
    "objective_fn"    : min_ess_objective,
...
FHMC_NO_FLIP = {
    "sampler_factory" : make_fhmc_unbiased_factory(DIM, reduced_flip=False),
    "objective_fn"    : min_ess_objective,
    "bounds"          : torch.tensor([[-3., 0.], [-0.3, 1.]]),
    "param_names"     : ["log10_gamma", "beta"],
    "discrete_dims"   : [],
...
def run_diagnostics(best_x: Tensor, cfg: dict, N_batch: int = 20, N_run: int = 500) -> dict:
    ...
    z_0      = torch.zeros(N_batch, cfg["state_dim"])
...
def run_acf_plots(optimal_params: dict, acf_kwargs: dict, max_lag: int = 400):
    ...
    dim_labels = [r"$v$", r"$\theta_1$", r"$\theta_5$"]
    dim_names  = ["v", "theta1", "theta5"]
    dim_indices = [0, 1, 5]

    model_name   = "funnel_benchmark2"
    ...
    for name, cfg in ALGORITHMS.items():
        ...
        z_0     = torch.zeros(acf_kwargs["N_batch"], cfg["state_dim"])
    ...
    acf_tag   = "dim{dim}_Nbatch{N_batch}_Nrun{N_run}".format(dim=DIM, **acf_kwargs)
...
def main():
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    bo_tag    = "dim{dim}_ninit{n_init}_nopt{n_opt}_Nbatch{N_batch}_Nrun{N_run}".format(dim=DIM, **BO_KWARGS)
