...
class FunnelNUTS:
    ...
    def __init__(self, dim: int, step_size: float, N_run: int, burn: int = 200):
        self.dim       = dim
...
def make_fhmc_unbiased_factory(dim: int, reduced_flip: bool = True):
    def factory(parameters: Tensor, N_run: int):
        ...
        return FunnelFHMCUnbiased(
            dim=dim, l=round(p[0].item()), N_fx=6,
...
NUTS_CONFIG = {
    "sampler_factory" : make_nuts_factory(DIM),
...
def run_acf_plots(optimal_params: dict, acf_kwargs: dict, max_lag: int = 400):
    ...
    for dim_idx, (dim_label, dim_name, state_idx) in enumerate(zip(dim_labels, dim_names, dim_indices)):
        ...
        ax.set_title(rf"ACF — Neal's funnel, dimension {dim_label}, $K={DIM}$")
