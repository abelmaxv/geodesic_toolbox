...
class RosenbrockHMC:
    def __init__(self, step_size: float, mass_variance: float, N_run: int, l: int = 1, burn: int = 200):
...
def run_acf_plots(optimal_params: dict, acf_kwargs: dict, max_lag: int = 400):
    ...
    acf_tag   = "dim2_Nbatch{N_batch}_Nrun{N_run}".format(**acf_kwargs)
...
def main():
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    bo_tag    = "dim2_ninit{n_init}_nopt{n_opt}_Nbatch{N_batch}_Nrun{N_run}".format(**BO_KWARGS)
