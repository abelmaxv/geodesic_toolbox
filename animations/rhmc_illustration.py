"""
chi-feng style illustration of *Riemannian* Hamiltonian Monte Carlo on the
Rosenbrock target.

Reuses the exact sampler from the internship experiments, `RosenbrockRHMC`
(internship_experiments/rosenbrock_benchmark2.py), which runs an implicit
leapfrog on the SoftAbs metric of the Rosenbrock potential

        U(x, y) = (100 (y - x^2)^2 + (1 - x)^2) / 20 ,

All parameters are set in this file's __main__ block (see the note there about
why they differ from the experiment's ESS-tuned operating point).

The key visual difference from plain HMC: the momentum is drawn from
N(0, g(z)) with a *position-dependent* metric, so the resulting initial-velocity
distribution  q_dot = g(z)^{-1} p ~ N(0, g(z)^{-1})  is an ellipse that stretches
along the local valley direction.  We draw that ellipse directly on the density
at the current point, so you can watch it tilt and stretch as the chain moves.

Run:
    python animations/rhmc_illustration.py
Output:
    animations/rhmc_illustration.mp4   (falls back to .gif if ffmpeg is missing)
"""
...
from rosenbrock_benchmark2 import RosenbrockRHMC  # noqa: E402
...
def make_animation(records, U_fn, l, gamma, xlim, ylim, out_path, title,
                   ...
                   xlabel="$x$", ylabel="$y$"):
    # The ellipse is the TRUE 1-sigma / 2-sigma covariance ellipse of the velocity
    # distribution v ~ N(0, g(z)^{-1} * std_0^2); with std_0 = 1 its size depends
    # only on the (SoftAbs) metric. vel_scale = 1 means data units = velocity units
    # (no artificial L*gamma scaling). The launch arrow is the realised velocity
    # sample drawn at the same scale, so it lands inside this distribution.
...
if __name__ == "__main__":
    # NOTE on parameters: the experiment's tuned operating point (gamma=0.57,
    # N_fx=6) is tuned for *sampling efficiency* (ESS per gradient), not for
    # accurate trajectories. At that step size the implicit leapfrog on the stiff
    # SoftAbs metric does not converge and the integrator diverges -- trajectories
    # blow up / U-turn, which is an integration artifact, NOT the true RHMC flow.
    # For a faithful *illustration* we integrate the same dynamics far more
    # accurately: a small step size and more fixed-point iterations. L*gamma is
    # kept moderate so the trajectory does not complete a half-oscillation in the
    # valley and double back. This yields the smooth gliding-along-the-valley flow
    # RHMC is supposed to produce (energy conserved to |dH| ~ 0.1, no U-turns).
    ...
    out_path = os.path.join(_HERE, "rhmc_illustration.mp4")
    make_animation(
